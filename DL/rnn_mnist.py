...
lr = 0.001
batch_size = 128
n_inputs = 28   # dimension of input x
...
def RNN(X):
    ...
    outputs = tf.unstack(outputs)  # remove first dimension, outputs: (batch_size,hidden_size)
    ...
    prediction = tf.nn.softmax(tf.matmul(outputs[-1], w2) + b2)
    return prediction
