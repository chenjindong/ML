...
import src_new.model.seq_rnn as seq_rnn
import src_new.model.seq_cnn as seq_cnn
import src_new.model.seq_dnn as seq_dnn
...
def ModelFactory(model_type, pre_embedding):
    if model_type == "rnn":
        return seq_rnn.SeqModelRNN(default_dim=FLAGS.default_dim,
        ...
        return seq_cnn.SeqModelCNN(default_dim=FLAGS.default_dim,
                                   seq_length=FLAGS.seq_length,
                                   song_size=FLAGS.song_size,
                                   singer_size=FLAGS.singer_size,
                                   sex_size=FLAGS.sex_size,
        ...
        return seq_dnn.SeqModelDNN(default_dim=FLAGS.default_dim,
                                   seq_length=FLAGS.seq_length,
                                   song_size=FLAGS.song_size,
                                   singer_size=FLAGS.singer_size,
                                   sex_size=FLAGS.sex_size,
...
def random_get_predict_label(features, is_neg):
    predict_labels = features['pos_label']
    if is_neg:
        predict_labels = features['neg_label']
...
def parse_record(record_proto):
    ...
    random_val = tf.keras.backend.random_uniform([1], 0, 1)
    neg_rate_tensor = tf.constant([FLAGS.neg_rate], dtype=tf.float32, shape=[1])

    binary_label = tf.cond(tf.math.less(random_val, neg_rate_tensor), lambda: tf.constant(0), lambda: tf.constant(1))
    features['predict_label'] = tf.cond(tf.math.less(random_val, neg_rate_tensor),
                                        lambda: random_get_predict_label(features, 1),
                                        lambda: random_get_predict_label(features, 0))
