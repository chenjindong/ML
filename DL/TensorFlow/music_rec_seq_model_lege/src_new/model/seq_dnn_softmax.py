...
class SeqModelDNNSoftmax(tf.keras.Model):
    def __init__(self, default_dim, seq_length, song_size, singer_size, sex_size, age_size, time_category_size,
        ...
        super(SeqModelDNNSoftmax, self).__init__()

        print("SeqModelDNNSoftmax init")
        ...
        self.dense_history = tf.keras.layers.Dense(units=default_dim, activation=tf.nn.relu)
    ...
    def call(self, inputs, training=False):
        ...
        embedding_history_flatten = self.flatten(embedding_history)
        embedding_history_dnn = self.dense_history(embedding_history_flatten)

        x = tf.concat([embedding_sex, embedding_age, embedding_current, embedding_history_dnn], 1)
