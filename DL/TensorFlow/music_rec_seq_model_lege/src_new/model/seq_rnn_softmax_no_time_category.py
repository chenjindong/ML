...
class SeqModelRNNSoftmaxNoTimeCategory(tf.keras.Model):
    def __init__(self, default_dim, seq_length, song_size, singer_size, sex_size, age_size, time_category_size,
        ...
        super(SeqModelRNNSoftmaxNoTimeCategory, self).__init__()

        print("SeqModelRNNSoftmaxNoTimeCategory init")
    ...
    def call(self, inputs, training=False):
        ...
        embedding_current = tf.concat(
            [embedding_current_song, embedding_current_singer, embedding_current_rate],
            1)

        embedding_current = self.dense_1(embedding_current)

        embedding_history = tf.concat(
            [embedding_history_song, embedding_history_singer, embedding_history_rate],
            2)
