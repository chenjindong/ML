...
class SeqModelCNNSoftmaxDeep(tf.keras.Model):
    def __init__(self, default_dim, seq_length, song_size, singer_size, sex_size, age_size, time_category_size,
        ...
        super(SeqModelCNNSoftmaxDeep, self).__init__()

        print("SeqModelCNNSoftmaxDeep init")
        ...
        self.conv = tf.keras.layers.Conv1D(default_dim, 3, activation=tf.nn.relu)
        ...
        self.dense_3 = tf.keras.layers.Dense(units=default_dim, activation=tf.nn.selu)
    ...
    def call(self, inputs, training=False):
        ...
        conv_history = self.conv(embedding_history)
        pooling_history = self.pooling(conv_history)

        x = tf.concat([embedding_sex, embedding_age, embedding_current, pooling_history], 1)
