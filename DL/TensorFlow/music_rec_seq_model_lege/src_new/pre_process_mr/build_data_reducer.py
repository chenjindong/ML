...
def get_vocab_size(vocab_dict, field_id):
    if field_id not in vocab_dict:
        ...
        return len(vocab_dict[field_id]) + 1
...
def gen_pos_labels(labels, rates, start, end):
    ...
    for i in range(start, end):
        if int(rates[i]) < 5 or labels[i] == "0":
            continue
...
def process(label_vocab_size, label_col_id, rate_col_id, seq_col_ids, point_col_ids, start_pos,
    ...
    for line in sys.stdin:
        ...
        try:
            ...
            if len(labels) < 200 or len(labels) > 5000:
                continue
...
if __name__ == "__main__":
    ...
    step_size = 5
    negative_size = 20
    process(label_vocab_size, label_col_id, rate_col_id, seq_col_ids, point_col_ids, start_pos, window_train,
            window_predict, step_size, negative_size)
